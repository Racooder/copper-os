...
--[[
Thread library v 2.0(?) by mr-V01d
https://github.com/mr-V01d/Treading-CC
...
function Threading.create(func)
...
function Threading.current()
	return threadPool[curentThreadId]
end

---Master thread, it should be called once, in the start of program/os
local function start()
	if started then
		error("Thread system already started", 2)
	end
	started = true
	local tFilters = {}
	local eventData = { n = 0 }
